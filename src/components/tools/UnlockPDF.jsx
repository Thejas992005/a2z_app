...
export const UnlockPDF = ({ onBack }) => {
  ...
  const executeUnlock = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || 'Failed to decrypt PDF document. Make sure the password is correct.');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="container">
      <ToolHeader title="Unlock PDF" onBack={onBack} />
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem 0' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-subtle)', padding: '2.5rem', borderRadius: '16px', maxWidth: '400px', width: '100%', textAlign: 'center' }}>
                  ...
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
                    Ready for decryption • {formatSize(file.size)}
                  </p>
                ...
                <div style={{ marginTop: 'auto', paddingTop: '1.5rem', borderTop: '1px solid var(--border-subtle)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.875rem' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <Loader2 className="animate-spin" size={14} /> Decrypting PDF document...
                    </span>
                    <span>{progress}%</span>
                  </div>
            ...
            {!file ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div className="form-group">
                  <label className="form-label">Document Password</label>
                  <div style={{ position: 'relative' }}>
                    ...
                    />
                    <Unlock size={14} style={{ position: 'absolute', left: '0.85rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
                  </div>
                </div>
